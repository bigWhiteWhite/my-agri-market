				...
				<div class="steps-title">
					...
					<div class="address">
						...
						<div class="user-address">
							<span>{{defaultAddress.region}}</span><span>{{defaultAddress.detailed}}</span>
						</div>
	...
	export default {
		...
		methods: {
			orderForm(){//提交订单
				...
				params.append('userInfoId',this.defaultAddress.id)
				this.$axios.post('/order/submit',params)
				.then(res=>{
					console.log(res.data.data)
					 this.$message({
					   message: '订单提交成功，正在跳转支付界面',
					   type: 'success'
					 });
					 let arr = res.data.data
					 this.$router.push({name:'Pay',query:{data:arr}})
				})
				.catch(err=>{
